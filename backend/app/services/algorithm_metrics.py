...
class AlgorithmMetricsService:
    ...
    def build(self) -> dict:
        ...
        return {
            ...
            "candidate_score_performance_7d": (
                self._candidate_score_performance_7d()
            ),
            # Backward-compatible key; underlying data is retained 7 days.
            "score_performance_30d": self._score_performance_7d(),
        }

    def _score_performance_7d(self) -> dict:
        records = self.paper_orders.recent(limit=1000, days=7)
        buckets = [
            ("60-69", 60, 70),
            ("70-79", 70, 80),
            ("80-100", 80, 101),
    ...
    def _candidate_score_performance_7d(self) -> dict:
        records = self.paper_orders.recent(limit=1000, days=7)
        buckets = [
            ("0-59", 0, 60),
            ("60-69", 60, 70),
        ...
        for label, lower, upper in buckets:
            ...
            for record in records:
                if (
                    ...
                    continue
                try:
                    score = float(record.get("candidate_score"))
                    pnl = float(record.get("realized_pnl"))
                    return_pct = float(
        ...
        return {
            ...
            "market": "stock",
            "total_closed_trades": total_closed,
            "minimum_bucket_sample_for_weight_review": 10,
            "buckets": result,
        }
