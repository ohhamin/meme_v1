...
BUY는 1회 고정 비중이 아니라 판단점수에 따른 목표 보유비중을 사용한다.
...
현재 보유가치가 목표보다 작을 때 그 차이만 추가 매수한다.
주식은 정수주 제약 때문에 계산금액이 1주보다 작아도 최소 1주 주문 후보를 만들 수 있으며,
Risk Guard가 현금 reserve와 종목 집중도 상한을 최종 검사한다.
...
- BUY가 종목 60% 한도나 최소 현금 10%를 일부만 초과하면 전부 BLOCK하지 않고 가능한 안전 수량까지 REDUCE한다.
