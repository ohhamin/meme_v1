...
- 한 종목은 해당 계좌 평가금액의 최대 60%까지 허용한다. 이 값은 목표비중이 아니라 Risk Guard의 절대 상한이다.
...
BUY sizing은 점수별 목표 보유비중 5/10/15/20%를 사용하고 현재 보유가치와의 차이만 주문 후보로 만든다. 주식은 정수주 제약 때문에 목표금액이 1주보다 작아도 최소 1주 후보를 만들 수 있으며, 60% 종목 상한·현금 10% reserve 등은 Risk Guard가 최종 검사한다. SELL은 보유수량의 25~60%를 단계적으로 줄인다.
